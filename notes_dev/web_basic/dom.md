...
- 当浏览器解析完HTML 文档后，每个节点并非只是一个单纯的`Element` 对象，而是继承了很多属性和方法，`Element` 对象继承了`Node`
  对象的所有属性和方法
...
- DOM API
    ...
    - 属性
        ...
        - `baseURI`：`string`，当前网页的绝对路径
            ...
            - 该属性的值一般由当前网址的 URL（即`window.location`属性）决定，但是可以使用 HTML 的`<base>`标签，改变该属性的值。设置了以后，
        ...
        - `firstChild`和`lastChild`：`Node`
            - `firstChild`：返回当前节点的第一个子节点，返回的可能是元素节点、文本节点或注释节点，如果当前节点没有子节点，则返回
              `null`
    ...
    - DOM节点常用操作
        ...
        - `cloneNode(cloneChild)`：`Node`，克隆一个节点
            ...
            - 会拷贝该节点的所有属性，但是会失去`addEventListener` 方法和`on-` 属性（例如`node.onclick = fn`
              ）添加在这个节点上的事件回调函数。
...
- DocumentFragment 节点
    ...
    - 用于构建一个 DOM 结构，然后插入当前文档
        ...
        ```javascript
		const docFrag = document.createDocumentFragment(); // (1)!
...
- keypress：按下有值的键时触发，即按下 Ctrl、Alt、Shift、Meta这样无值的键，这个事件不会触发。对于有值的键，按下时先触发`keydown`
  事件，再触发这个事件
- keyup：松开键盘时触发该事件
...
- `input`事件
    - 当`<input>` 、`<select>` 、`<textarea>` 的值发生变化时触发，对于复选框（`<input type=checkbox>`）或单选框（
      `<input type=radio>`），用户改变选项时，也会触发这个事件。
...
- 浏览网页就是一个加载各种资源的过程 ，图像（image）、样式表（stylesheet）、脚本（script）、视频（video）、音频（audio）、Ajax
  请求（XMLHttpRequest）等等。这些资源和`document`对象、`window`对象都会触发`load`事件
...
- 窗口关系：`window.top`，`window.parent`，`window.self`
    ...
    - 窗口大小：`window.[inner|outer][Width|Height]`，`document.body.client[Width|Height]`，`resizeTo(width,height)`，
