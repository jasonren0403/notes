...
* DOM标准
    ...
    === "DOM Level 4"
		url：https://www.w3.org/TR/dom41/  仍然处于草案阶段  
...
### DOM的对象与接口
* 当浏览器解析完HTML 文档后，每个节点并非只是一个单纯的Element 对象，而是继承了很多属性和方法，Element 对象继承了Node 对象的所有属性和方法
...
* DOM API
	- 属性
		...
		* `nodeValue`：`string`，该节点本身的文本值，可以读写
			- 只有text、comment、attr节点有文本值，有返回结果，可设置属性。
		...
		* `ownerDocument`：`Document`，返回当前节点所在的顶层文档对象，即document 对象
	...
	- DOM节点常用操作
		...
		* `cloneNode(cloneChild)`：`Node`，克隆一个节点
			...
			- 会拷贝该节点的所有属性，但是会失去`addEventListener` 方法和on- 属性（例如`node.onclick = fn`）添加在这个节点上的事件回调函数。
...
* Document 节点对象，代表整个文档
	...
	- 便捷访问文档内部的某个节点：快捷方式属性
		* 例如`document.body`属性指向`<body>`节点，`document.head`属性指向`<head>`节点，`document.activeElement`属性返回获得当前焦点（focus）的 DOM 元素，`document.fullscreenElement` 属性返回当前以全屏状态展示的 DOM 元素......
	- 节点集合属性
		...
		* 一些属性
			...
			* `document.embeds` 属性、`plugins` 属性返回所有`<embed>`节点
			* `document.scripts` 属性返回所有`<script>`节点
			* `document.styleSheets` 属性返回文档内嵌或引入的样式表
	- 文档静态信息属性
		* `document.documentURI` 属性和`document.URL` 属性返回一个字符串，表示当前文档的网址
		* `document.domain` 属性返回当前文档的域名，不包含协议和接口
		* `document.lastModified` 属性返回当前文档最后修改时间的字符串
		* `document.title` 属性返回当前文档的标题，该属性是可写的
		* `document.characterSet` 属性返回当前文档的编码
	- 文档状态属性
		* `document.hidden` 属性返回布尔值，表示当前页面是否可见
		...
		* `document.readyState`返回当前文档的状态
			...
			- `complete`：加载完毕
	- 对象方法
		* `document.querySelector` 方法接受一个 CSS 选择器作为参数，返回第一个匹配该选择器的元素节点，如果没有则返回`null`
		* `document.querySelectorAll` 方法返回一个`NodeList`对象，包含所有匹配给定选择器的节点
		* `document.getElementsByTagName` 方法搜索HTML 标签名，返回符合条件的元素
			* 返回值是一个类似数组对象（HTMLCollection实例），可以实时反映 HTML 文档的变化
			* 如果没有任何匹配的元素，就返回一个空集
		* `document.getElementsByClassName` 方法返回所有`class` 名字符合指定条件的元素，元素的变化实时反映在返回结果中
		* `document.getElementsByName` 方法用于选择所有拥有`name` 属性的 HTML 元素
	...
	- 事件相关：`createEvent()` `addEventListener()` `removeEventListener()` `dispatchEvent()`
		```javascript
		/* 实例代码  */
			...
			- 操作属性：`getAttribute()`、`getAttributeNames()`、`setAttribute()`、`hasAttribute()`、`hasAttributes()`、`removeAttribute()`
				...
				!!! summary "使用JavaScript动态操作CSS"  
                    ...
                    === "操作Style属性"  
						...
						```
						相当于```<div style="background-color:red;border:1px solid black;"/>```
...
* DocumentFragment 节点
	...
	- 用于构建一个 DOM 结构，然后插入当前文档
		...
		//可以用new创建，也可以用document.createDocumentFragment()来创建
		var li = document.createElement('li');
		li.textContent = 'hello world';
		docFrag.appendChild(li);
		document.querySelector('ul').appendChild(docFrag);
		//上述方法中，docFrag在添加结束后会被清空(textContent为'')，如果想保留其中内容，使用cloneNode()方法
...
2. 设置元素节点对象的事件属性指定监听函数`window.onload = doSomething;`
	* 这种方法与 HTML 的on- 属性的差异是，它的值是函数名（`doSomething`），而不必给出完整的监听代码（`doSomething()`）
	* 只在冒泡阶段触发
3. 所有 DOM 节点都有`addEventListener`方法，用来为该节点定义事件的监听函数
	...
	* 注意：IE8无`addEventListener`方法，但有类似的`attachEvent()`方法，可以为同一事件绑定多个事件处理函数，但是后绑定的先执行
...
- mousewheel：滚动鼠标的滚轮时触发，该事件继承的是WheelEvent 接口
	...
	* 注意：火狐浏览器使用`DOMMouseScroll`来添加滚动事件，且需要使用`addEventListener()`
