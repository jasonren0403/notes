...
- CSS的技术发展
    ...
    === "CSS2(1998)"
        * CSS2推荐的是一套内容和表现效果分离的方式，HTML元素可以通过CSS2的样式控制显示效果，可完全不使用以往HTML中的`table`和`td`来定位表单的外观和样式，只需使用`div`和`li`此类HTML标签来分割元素，之后即可通过CSS2样式来定义表单界面的外观
...
- 利用CSS控制样式
    ...
    * 利用不可见的container
    * 利用父子节点（[层叠和继承](#语法详解层叠cascade与继承inheritance)）
...
=== "状态伪类（链接）<a id='state'></a>"

    === "`:link`"
        表示链接的正常状态，选择那些尚未被点过的链接

    === "`:visited`"
        选择点过的链接

    === "`:focus`"
        选择已经通过指针设备、触摸或键盘获得焦点的元素，在表单里使用得非常多
...
=== "验证伪类（表单）"
    ...
    === "启用状态"
        `:disabled`选择禁用状态的表单元素，`:enabled`启用状态
...
* `::before` `::after` 为其他HTML元素添加内容（文本或图形），添加的内容不实际存在于DOM中，但可以像存在一样操作它们，需要在CSS中声明`content`属性
* `::first-letter`  选择一行文本第一个字符，并且文字所处的行之前没有其他内容（如图片和内联的表格）
* `::first-line`  选择元素的第一行，只适用于块级元素，内联元素不适用
* `::selection`  选择文档中被高亮选中的部分，基于Gecko的浏览器要使用`::-moz-selection`

!!! note ""
    * `first-letter`只对块级元素生效，而`before`和`after`没有限制
	* 可以像操作正常html文档元素一样来操作它们。
...
=== "<a id="text-align"></a>对齐"
	- 语法：`text-align:left|right|center|justify;`
	- 控制文本水平方向的对齐方式
		* justify：使文本对齐在两端，所有文本行宽度相同
		* 只对文本文字和img标签有效

=== "<a id="line-height"></a>行高"
	- 语法：`line-height:属性值;`
	- 值：数字、长度、百分比、关键字normal
	- `line-height = font-size + leading`，其中leading是填充在两行文字间的“铅条”
...
#### 背景样式
* <a id="bg-color"></a>background-color
...
* <a id="bg-pos"></a>background-position
	...
	- 属性值
		...
		* 百分比（`水平 垂直`）
			...
			- 若仅规定了一个值，则另一个值是50%
		* position值（`水平 垂直`）
			- 左上角位置为（0，0），单位为任何CSS单位
            - 若仅规定了一个值，则另一个值为50%
			- position值可以与百分比值混用

* <a id="bg-size"></a>background-size
...
* <a id="bg-attachment"></a>background-attachment
...
- 层叠优先级算法
	1. 针对某一元素的某一属性，列出所有给该属性定值的CSS规则
		...
		!!! info "CSS样式可能都来自哪里"
			1. <a href="../html#apply-css-and-js-in-webpage">作者样式表</a>（外部样式表、内部样式表、内联样式表）
			2. 浏览器默认样式
			3. 用户样式表
	...
	4. 根据CSS规则在样式文档中出现的位置（Source Order）进行优先级排序
        ...
        <a id="specificity">选择器特殊性</a>：style标签（1000） > ID选择器（100） > 类、属性、伪类选择器（10） > 元素选择器、伪元素选择器（1）
...
- 块级元素、行内元素和盒模型
	...
	=== "块级元素<a id='block-element'></a>"
		* 有宽、高，可定义四个方向的`margin`
		* 默认从上到下排列，占据整行

    === "行内元素<a id='inline-element'></a>"
		* 没有宽、高，可定义左右两个方向的`margin`

		!!! info ""
			但是行内块元素（inline-block）是有宽和高属性的
