...
## HTML元素
* Element = opening tag(example：`<p>`) + closing tag(example:`</p>`) + content
* 大小写不敏感
* 可以嵌套  `<p><span>some text.</span>some other text.</p>`
* 存在自闭合元素 `<hr />` `<meta />`等
* 属性  
	- 属性名称=属性值  例如：`<p class = "editor-note"></p>`
    - 布尔属性  例如：`<input type="text" disabled>`
* 块级元素 vs 内联元素
    - 块级元素 `<p>` `<h1>-<h6>` `<div>` 列表和列表项 `<form>`等 会导致文本换行
    - 内联元素 `<span>` `<img>` `<a>`等 不会导致文本换行
...
4. 多媒体和嵌入
    * <a name="html-pic">图片</a>  
    	* 语法：```<img src="xxx" alt="yyy" title="zzz" />```   
		* 属性
			* title 图片描述（给用户看） 
    ...
    * <a name="html-video">视频</a>  
		* 语法：```<video src="xxx" [properties]> </video>```  
		* 属性
			- src 路径  
    ...
    * <a name="html-audio">音频</a>  
        * 语法：```<audio [properties]> <source src="xx" type="yy">...</audio>``` 
		* 用法与`<video>`标签类似  
...
5. 超链接```<a href="" title="" target="" [download=""]></a>```  
...
6. 表单（form）
	...
	* 语法：`<form action="action_page_path" name="" [method="" target="" accept-charset="" enctype="" autocomplete="" novalidate]>`
		- 属性
			...
			* method：使用哪一种HTTP提交方法发送数据，取值为GET或POST  
			...
			!!! info "GET vs POST"  
				GET通过URL提交数据，表单数据可见`http://someweb/?param1=a&param2=b&...`，提交数据量受浏览器和web服务器限制（URL长度相关）  
	...
	* 表单元素
		- `<input type="xx">`元素  
			...
			* 复选框checkbox  <input type="checkbox" value="value" name="group2" checked="checked"/>  
			`<input type="checkbox" value="复选框取值" name="所在组名" checked[="checked"] /> `  
				
			!!! tip "提示"  
				复选框的文本要加入label标签，用for指向该复选框的id(`label for="xx"`)
								
			* 文件选择器 <input type="file" name="file" id="file" accept="image/*" multiple>  
			`<input type="file" name="file" id="file" accept="accept-file-mime" multiple>`  
				- 接受文件类型：`accept`属性
				- 可选多个文件：`multiple`属性
